...
public class DictionaryConnection2 {
     ...
     */
    public DictionaryConnection2(String host, int port) throws DictConnectionException {
        try {
     ...
     *                                 don't match their expected value.
     */
    public DictionaryConnection2(String host) throws DictConnectionException, IOException {
...
//            if (readStatus(in).getStatusCode() != 250) throw new DictConnectionException();
    ...
    public synchronized Collection<Definition> getDefinitions(String word, Database database) throws DictConnectionException {
        ...
        try {
            ...
            output.print(msg);
            status = Status.readStatus(in);
            String[] firstResponse = DictStringParser.splitAtoms(readStatus(in).getDetails());
            int numDefinitions = Integer.parseInt(firstResponse[0]);
            int i = 0;
            if (status.getStatusCode() != 150) {
                if (status.getStatusCode() == 552) { // return empty set
                    return set;
                }
                throw new DictConnectionException();
            }
            while (i < numDefinitions){
                String[] line = DictStringParser.splitAtoms(in.readLine());
                Definition def = new Definition(word, line[0]); // HOPE THIS WORKS
